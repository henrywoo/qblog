...
@login_required
def monitor(request):
    return render(request, 'sentosa/monitor.html')
...
def index(request):
    ...
    for s in SYMBOLS:
        ti[s] = getTI(s)
    return render(request, 'sentosa/sentosa.html',
                  {#'positions': positions,
                   'ti': ti,
...
# http://192.168.254.130/sentosa/tobj/YY/
@login_required
def tobj(request, sym):
    tinfo = getTI(sym)
    return render(request, 'sentosa/tobj.html',
                  {'tinfo': tinfo,
                   'sym' : sym,
                   'POSITION_STATUS': POSITION_STATUS,}
                  )
...
def symbol(request, sym):
    tinfo = getTI(sym)
    return render(request, 'sentosa/symbol.html',
                  {'tinfo': tinfo,
                   'sym' : sym,
                   'POSITION_STATUS': POSITION_STATUS,}
                  )
...
def data(request, sym, callback):
    ...
    if datacache.has_key(sym):
        ...
        datacache[sym] = d
    return render(request, 'sentosa/data.html',{'d': callback + d})
    #return HttpResponse("parent.Response_OK()", mimetype="application/x-javascript")

def error404(request):
    #return render(request, 'sentosa/404.html')
    from django.template import Context, loader
    template = loader.get_template('sentosa/404.html')
