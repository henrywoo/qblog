/**
 * marked2 - a markdown parser
 * Copyright (c) 2011-2013, Christopher Jeffrey. (MIT Licensed)
 * https://github.com/chjj/marked2
 */
...
();block.tables=merge({},block.gfm,{nptable:/^ *(\S.*\|.*)\n *([-:]+ *\|[-| :]*)\n((?:.*\|.*(?:\n|$))*)\n*/,table:/^ *\|(.+)\n *\|( *[-:]+[-| :]*)\n((?: *\|.*(?:\n|$))*)\n*/});function Lexer(options){this.tokens=[];this.tokens.links={};this.options=options||marked2.defaults;this.rules=block.normal;if(this.options.gfm){if(this.options.tables){this.rules=block.tables;}else{this.rules=block.gfm;}}}
...
(']|','~]|')
('|','|https?://|')
()});inline.breaks=merge({},inline.gfm,{br:replace(inline.br)('{2,}','*')(),text:replace(inline.gfm.text)('{2,}','*')()});function InlineLexer(links,options){this.options=options||marked2.defaults;this.links=links;this.rules=inline.normal;if(!this.links){throw new
...
out+='&#'+ch+';';}
return out;};function Parser(options){this.tokens=[];this.token=null;this.options=options||marked2.defaults;}
...
return obj;}
function marked2(src,opt){try{if(opt)opt=merge({},marked2.defaults,opt);return Parser.parse(Lexer.lex(src,opt),opt);}catch(e){e.message+='\nPlease report this to https://github.com/chjj/marked2.';if((opt||marked2.defaults).silent){return'An error occured:\n'+e.message;}
throw e;}}
marked2.options=marked2.setOptions=function(opt){merge(marked2.defaults,opt);return marked2;};marked2.defaults={gfm:true,tables:true,breaks:false,pedantic:false,sanitize:false,smartLists:false,silent:false,highlight:null,langPrefix:'lang-'};marked2.Parser=Parser;marked2.parser=Parser.parse;marked2.Lexer=Lexer;marked2.lexer=Lexer.lex;marked2.InlineLexer=InlineLexer;marked2.inlineLexer=InlineLexer.output;marked2.parse=marked2;if(typeof exports==='object'){module.exports=marked2;}else if(typeof define==='function'&&define.amd){define(function(){return marked2;});}else{this.marked2=marked2;}}).call(function(){return this||(typeof window!=='undefined'?window:global);}());
...
;(function(window, document) {
  ...
  // Generate Markdown
  var html = marked2(markdown);
  //$("#FuhengMarkDown").innerHTML = html;
  document.getElementById('FuhengMarkDown').innerHTML = html;
